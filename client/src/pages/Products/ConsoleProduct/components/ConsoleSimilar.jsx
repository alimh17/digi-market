...
const ConsoleSimilar = (props) => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const location = useLocation();

  let consoles = useSelector((state) => state.consoles.allProduct);
  const filter = consoles.filter(
    ...
    <section className="font-sans">
      ...
      >
        {filter.map((item) => (
          <SplideSlide
            key={item._id}
            className="flex justify-center items-center flex-col  rounded-lg h-96 border-2 cursor-pointer"
            onClick={() => {
